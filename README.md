...
- **Alt + 右键连接**：按住 `Alt` 右键点一下源节点，再右键点一下目标节点；也可以从源节点直接右键拖到目标节点。插件会根据类型、插槽名称和占用状态选择连接。
...
- **命令菜单**：在画布或节点右键菜单选择 **Flow Wrangler**。

`Alt+C` 是新版 ComfyUI 自带的“折叠/展开所选节点”，`Ctrl+Alt+C` 经常被系统截图工具占用，`Ctrl+Shift+W` 则是 Chrome/Edge 的“关闭窗口”。Flow Wrangler 不使用这些组合。所有命令都会显示在 **设置 → 快捷键** 中，可以搜索 `Flow Wrangler` 后自行绑定。
